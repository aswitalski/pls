  ...
  useEffect(() => {
    ...
    async function process(svc: typeof service) {
      ...
      try {
              ...
              prev.map((item, idx) => {
                if (idx === progress.currentIndex) {
                  ...
                  return {
                    ...
                    endTime: isEnding ? now : item.endTime,
                  };
                }
                return item;
              })
        ...
        if (mounted) {
          setIsExecuting(false);
          // Calculate total elapsed from all command times
          // Floor each to seconds before summing so total matches displayed values
          setCommandStatuses((prev) => {
            const totalElapsed = prev.reduce((sum, cmd) => {
              if (cmd.startTime && cmd.endTime) {
                const elapsedMs = cmd.endTime - cmd.startTime;
                const flooredMs = Math.floor(elapsedMs / 1000) * 1000;
                return sum + flooredMs;
              }
              return sum;
            }, 0);
