...
export function Output({ stdout, stderr, failed }: OutputProps) {
  ...
  const stderrColor = failed ? Palette.Yellow : Palette.Gray;

  return (
    <Box marginTop={1} marginLeft={5} flexDirection="column" width={MAX_WIDTH}>
      {shouldShowStdout &&
        stdoutLines.map((line, index) => (
          <Text key={`out-${index}`} color={Palette.Gray} wrap={wrapMode}>
            {line}
          </Text>
        ))}
